...
class ComponentPatients extends Component {
  ...
  Search = (evt) => {
    ...
    if (criteriobusqueda !== "") {
      pacientes = this.props.pacientes.filter(
        (p) => p.fechaDeCreacion.includes(value) || p.nombre.includes(value) || p.apellidos.includes(value) || p.ci.includes(value) || p.direccion.includes(value) || p.sexo.includes(value) || (p.historiaclinica != null ? p.historiaclinica.numerohistoria.includes(value) : false)
      );
      this.setState({
        pacientes: pacientes,
