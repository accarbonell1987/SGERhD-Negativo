...
class ComponentUsers extends Component {
	...
	state = { usuarios: null, criteriobusqueda: "" };
	...
	Search = (evt) => {
		...
		if (criteriobusqueda !== "") {
			usuarios = this.props.usuarios.filter((p) => p.nombre.includes(value) || p.rol.includes(value) || p.email.includes(value));
			this.setState({
				usuarios: usuarios,
	...
	render() {
		...
		if (this.state.criteriobusqueda !== "") usuarios = this.state.usuarios;
								...
								return (
									<Table.Row key={usuario._id} negative={negative}>
										...
										<Table.Cell className="cell-acciones" collapsing>
											...
											{this.props.global.rol === "informatico" ? <ComponentChangePassword usuario={usuario} gestion={true} global={this.props.global} /> : <Button icon="key" disabled />}
										</Table.Cell>
									</Table.Row>
								);
							})}
