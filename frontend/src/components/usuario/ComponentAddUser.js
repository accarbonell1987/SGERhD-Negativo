...
class ComponentAddUser extends Component {
	...
	AddUser = async () => {
		...
		if (!data) this.props.Deslogin();
			...
			let { nombre, contraseña, repetircontraseña, email, rol, activo } = this.state;
			//encriptar md5
			...
			<Modal open={this.state.openModal} trigger={accesomenu.permisos.crear ? this.ChangeIconInAddButton(true, this.props.cambiarIcono) : this.ChangeIconInAddButton(false, this.props.cambiarIcono)}>
				...
				<Modal.Content>
					...
					<Form ref="form" OnSubmit={this.ChangeModalState}>
						<Form.Input required name="nombre" icon="user" iconPosition="left" label="Nombre:" placeholder="nombre de usuario" error={this.state.errornombre} onChange={this.ChangeModalInput} onKeyDown={this.OnPressEnter} />
						<Form.Input required name="email" icon="mail" iconPosition="left" label="Correo Electrónico:" value={this.state.email} error={this.state.erroremail} placeholder="correo@host.com" onChange={this.ChangeModalInput} onKeyDown={this.OnPressEnter} />
						<Form.Select
							required
							name="rol"
							label="Rol:"
							...
							clearable
						/>
						<Form.Input required name="contraseña" icon="lock" iconPosition="left" label="Contraseña:" value={this.state.contraseña} error={this.state.errorcontraseña} type="password" onChange={this.ChangeModalInput} onKeyDown={this.OnPressEnter} />
						<Form.Input required name="repetircontraseña" icon="lock" iconPosition="left" label="Repetir Contraseña:" value={this.state.repetircontraseña} error={this.state.errorcontraseñacoincide} type="password" onChange={this.ChangeModalInput} onKeyDown={this.OnPressEnter} />
					</Form>
				</Modal.Content>
				<Modal.Actions>
					...
					<Button color="green" onClick={this.ChangeModalState} className="modal-button-acept" type="submit" disabled={!this.state.nombre || !this.state.email || !this.state.rol || !this.state.contraseña || !this.state.repetircontraseña}>
