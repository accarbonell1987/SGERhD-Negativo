...
class ComponentChangePassword extends Component {
	...
	//modificar usuario
	updateUser = async (id) => {
		const md5hash = md5(this.state.contraseña);
		//encriptar a md5
		const usuario = { contraseña: md5hash };
	...
	ChangeModalState = async (evt) => {
		if (evt.target.className.includes("modal-button-change-password") || evt.target.className.includes("modal-icon")) {
			...
			this.props.gestion ? this.ClearModalState() : this.props.ChangeModalState(false);
		} else {
			...
			if (this.HandleSubmit(evt) === false) {
				...
				if (!this.props.gestion) {
					...
					//encriptar md5 y luego comparar
					const md5hash = md5(this.state.contraseñaanterior);
					if (this.props.usuario.contraseña === md5hash) {
	...
	render() {
		return (
			<Modal open={this.props.gestion ? this.state.openModal : this.props.openModalPassword} trigger={this.props.gestion ? this.trigger : ""}>
